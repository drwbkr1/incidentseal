#!/usr/bin/env python3
"""Run dashboard meta-validation through the existing exact source gate."""
...
def run(root: Path, wheel_dir: Path | None) -> dict:
    ...
    receipts = []
    temporary_path = None
    with tempfile.TemporaryDirectory(prefix="incidentseal-dashboard-meta-") as temporary:
        ...
        completed = subprocess.run(
            [sys.executable, "-S", str(root / "scripts" / "validate_dashboard_schema_meta.py"), "--root", str(root)],
            cwd=root, env=environment, capture_output=True, timeout=60, check=False,
        )
        if completed.returncode != 0 or completed.stderr:
            detail = completed.stderr.decode("utf-8", errors="replace").strip()
            raise RuntimeError(f"dashboard meta-validator failed: {detail}")
        result = json.loads(completed.stdout)
    if temporary_path is None or temporary_path.exists():
        raise RuntimeError("dashboard meta-validation custody remained")
    result.update({
