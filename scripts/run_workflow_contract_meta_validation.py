#!/usr/bin/env python3
"""Run workflow execution contract meta-validation through the existing exact source gate."""
...
def run(root: Path, wheel_dir: Path | None) -> dict[str, object]:
    ...
    with tempfile.TemporaryDirectory(prefix="incidentseal-workflow-contract-meta-") as temporary:
        ...
        completed = subprocess.run(
            [sys.executable, "-S", str(root / "scripts" / "validate_workflow_contract_schema_meta.py"), "--root", str(root)],
            cwd=root, env=environment, capture_output=True, timeout=60, check=False,
        )
        if completed.returncode != 0 or completed.stderr:
            detail = completed.stderr.decode("utf-8", errors="replace").strip()
            raise RuntimeError(f"workflow contract meta-validator failed: {detail}")
        result = json.loads(completed.stdout)
    if temporary_path is None or temporary_path.exists():
        raise RuntimeError("workflow contract meta-validation custody remained")
    result.update({
...
def main() -> int:
    ...
    try:
        ...
        print(json.dumps({"schema_version":"incidentseal-workflow-execution-meta-validation/v1","verification_verdict":"INVALID","error":f"{type(error).__name__}: {error}"}, sort_keys=True, separators=(",", ":")))
        return 1
