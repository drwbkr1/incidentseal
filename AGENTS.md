...
- `requirements/dashboard-contract.lock.json` freezes only the runtime-free `IS5-U01` projection, serving, visual, scenario, and evaluation contract. It does not authorize starting a server or browser. The future dashboard uses a separate `incidentseal-dashboard` launcher so the frozen verification CLI remains unchanged. It must bind only IPv4 `127.0.0.1`, admit only fixed `GET` and `HEAD` routes, use only local assets and exact allowlisted source records, and have no Docker, approval-write, workflow, repository-write, secret, external-network, analytics, or telemetry authority. Rendered state is never verification or release authority.
