...
- `requirements/recovery-contract.lock.json` currently freezes classification only. Until the separate host-only recovery surface passes real runtime gates, do not stop a process, append recovery evidence, resume a step, or claim recovery from this contract. Active, missing, invalid, ambiguous, or unowned custody must defer.
