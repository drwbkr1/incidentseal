...
- `verify --manifest PATH --json` may execute only after external policy inspection is `MATCH` for the exact manifest, repository, and path. It supports the frozen Python and Node profile only; do not use it as a general host-command surface or attempt to select approval, image, state, network, mount, secret, or Docker custody.
- `run events --run-id ID --jsonl` is a read-only exact-byte stream from restrictive external workflow custody when that run exists, otherwise from one active digest-bound journal database. Its final event preserves completed verdict exits and distinct cancelled, failed, stale, and superseded lifecycle exits. The agent-facing CLI has no append command.
...
- `requirements/dashboard-evaluation.lock.json` binds the fixed U04 evaluator once present. Only `python -B scripts/run_dashboard_evaluation.py` may run the repeated surface. It accepts no scenario, repetition, route, repository, network, Docker, approval, workflow, output, or performance-threshold argument. Require all nine frozen states in order for three repetitions, 27 new loopback servers with five exact GETs each, exact state and claim calibration, seven-record coverage per trial, stable per-scenario HTML, crash-to-recovery checks, complete teardown, and public replay. Server-side render latency is not browser paint evidence.
...
- Do not infer approval from a valid manifest or digest. Only an external `policy status` value `MATCH` for the same exact manifest can satisfy the `verify` gate. Missing, mismatched, expired, invalid, or changed authority must stop before Docker or become a retained stale run if authority changes after start.
