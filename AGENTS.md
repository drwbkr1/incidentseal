...
- `requirements/backup-restore-contract.lock.json` freezes the public-reproduced receipt classification only. Until a separate host-only implementation passes real gates, do not claim backup/restore from the contract or mount protected volumes. A dump alone is never PASS; the fixed implementation must restore the exact archive into different clean disposable custody and reproduce schema, journal, result, role, privilege, restart, protection, and teardown evidence.
