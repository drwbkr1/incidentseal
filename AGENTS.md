...
- `requirements/backup-restore-contract.lock.json` currently freezes receipt classification only. Until a separate host-only implementation passes real gates, do not run a dump or restore, mount protected volumes, or claim backup/restore from the contract. A dump alone is never PASS; the exact archive must restore into different clean disposable custody and reproduce schema, journal, result, role, privilege, restart, protection, and teardown evidence.
