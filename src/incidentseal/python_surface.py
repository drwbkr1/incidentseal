"""Host-owned real Python application runner probe."""
...
def _expected_result(request: dict[str, Any]) -> dict[str, Any]:
    ...
    return {
        ...
        "result_digest": _sha((input_digest + "|python").encode("utf-8")),
...
def _raw_compose(
    ...
    try:
        ...
        raise TopologyError("IS_PYTHON_RUNTIME", "Python runner command could not execute", io_error=True) from error
...
def _container_isolation(docker: str, name: str, custody: Path, network: str) -> dict[str, Any]:
    ...
    expected_output = (custody / "python-output").resolve(strict=True)
    ...
    if not passed:
        raise TopologyError("IS_PYTHON_ISOLATION", "Python runner staged custody or isolation differs from the contract")
    return {
...
def python_probe() -> dict[str, Any]:
    ...
    with tempfile.TemporaryDirectory(prefix="incidentseal-python-") as temporary:
        ...
        if ROOT in custody.parents or custody == ROOT or any(part.casefold() == "onedrive" for part in custody.parts):
            raise TopologyError("IS_PYTHON_CUSTODY", "Python probe custody overlaps a forbidden root")
        for name in ("input", "python-output", "node-output"):
            (custody / name).mkdir()
        request_path = custody / "input" / "request.json"
        output_path = custody / "python-output" / "result.json"
        ...
        migration_name = f"{project}-migration-python-surface"
        positive_name = f"{project}-python-real"
        invalid_name = f"{project}-python-invalid"
        ...
        if _run(docker, ["ps", "-aq", "--filter", f"name={project}"]).stdout.strip():
            raise TopologyError("IS_PYTHON_STALE", "pre-existing Python probe container conflicts with the run")
        if _run(docker, ["network", "ls", "-q", "--filter", f"name=^{network}$"]).stdout.strip():
            raise TopologyError("IS_PYTHON_STALE", "pre-existing Python probe network conflicts with the run")
        if not _run(docker, ["volume", "ls", "-q", "--filter", f"name=^{volume}$"]).stdout.strip():
            raise TopologyError("IS_PYTHON_STALE", "the exact verified database volume is unavailable")
        ...
        try:
            ...
            expected = _expected_result(valid_request)
            positive = _raw_compose(
                docker,
                base,
                env,
                ["run", "--name", positive_name, "--no-deps", "python-runner"],
            )
            ...
            inspections.append(_inspect_container(docker, positive_name, identities["python-runner"], "65532:65532", network))
            ...
            if output_path.is_file():
                try:
                    ...
                    actual = None
            output_ok = actual == expected and set(custody.joinpath("python-output").iterdir()) == {output_path}
            ...
            row_sql = f"""SELECT COALESCE((SELECT json_build_object(
              'run_id', run_id, 'runner', runner, 'input_digest', input_digest, 'result_digest', result_digest
            ) FROM public.verification_results
            WHERE run_id = '{valid_request['run_id']}' AND runner = 'python'), json_build_object('missing', true))::text;"""
            row = _last_json(
                _run_psql(docker, base, env, f"{project}-query-python-row", admin_user, row_sql, created_names)
            )
            _check(checks, "database-row", row == {key: expected[key] for key in ("run_id", "runner", "input_digest", "result_digest")}, row)
            evidence["database_row"] = row
            _remove_completed(docker, created_names)
            ...
            invalid = _raw_compose(
                ...
                base,
                env,
                ["run", "--name", invalid_name, "--no-deps", "python-runner"],
            )
            created_names.append(invalid_name)
            invalid_text = (invalid.stdout + invalid.stderr).strip()
            inspections.append(_inspect_container(docker, invalid_name, identities["python-runner"], "65532:65532", network))
            ...
            absent_sql = f"""SELECT json_build_object('row_count', count(*))::text
              FROM public.verification_results WHERE run_id = '{invalid_request['run_id']}' AND runner = 'python';"""
            absent = _last_json(
                _run_psql(docker, base, env, f"{project}-query-python-invalid", admin_user, absent_sql, created_names)
            )
        ...
        if not teardown_ok:
            raise TopologyError("IS_PYTHON_TEARDOWN", "Python probe teardown or retained volume differs from the contract")
        verdict = "PASS" if all(item["status"] == "PASS" for item in checks) else "FAIL"
        return {
            "schema_version": "incidentseal-python-surface-probe/v1",
            "verdict": verdict,
            "mode": "platform-validation",
            "claim_scope": "python-application-runner-only",
