...
- Candidate immutable PostgreSQL event journal with per-run transaction serialization, exact-byte replay, unique conflict gates, mutation-denial triggers, read-only canonical `run events` JSONL, real stale/superseded exits, restart persistence, least privilege, and protected disposable custody.
