...
| A stale or competing recovery process stops another workflow's runtime | Separate workflow and recovery fence tokens; exact container ID, image, labels, user, mounts, network, and hardening; reobserve before every action | Exercise active owner, unowned lookalike, expired lease, competing recoverer, stopped runtime, and crash-resume cases against real Docker and PostgreSQL |
...
| Logical dump executes untrusted source SQL or restores elevated roles | Fixed synthetic source, exact archive and normalized TOC identity, no owner/ACL restore, roles rebuilt by locked migration, clean distinct target, error-stopping single transaction | Substitute archive/TOC, inject role or privilege drift, reuse source target, corrupt state, and require negative privilege probes after restore |
