...
## 2026-08-11 - The v0.1.0 contract freezes before any artifact build

U01 now binds plan digest `sha256:6c43b61a...` and release lock `sha256:8b1254d9...`. The contract deliberately inserts `incidentseal verify --manifest PATH --json` before packaging: the product promise already says an operator-approved manifest drives the real verification surfaces, but the checkout previously exposed only policy inspection and bounded platform probes. The added prerequisite keeps approval external, makes the agent incapable of approving its own policy, and preserves host ownership of Docker plus the socket, secret, network, privilege, and mount denials.

The frozen release remains compact: dependency-free wheel and source distribution, four linux/amd64 GHCR roles, nine downloadable assets, thirteen real surfaces, eight full-commit Action pins, SBOM, provenance, vulnerability and hardening evidence, repeated builds, exact digests, and credential-free consumer replay. PyPI and broader platform work remain excluded. A metadata-only source gate records nine build wheels plus exact Syft and Grype releases without acquisition or execution; redistribution remains unresolved until notices and every `NOASSERTION` entry close.

The candidate passes 141 tests, 53 release mutations, its exact lock, dependency-free validation, and one-schema/two-fixture full Draft 2020-12 validation from six retained exact wheels without network. Policy invocation `b0e1eb11...` remained `MISSING` / `INVALID` at exit `12`; no workflow, build, image, registry, or release action occurred. Docker has zero IncidentSeal containers and networks and the same three protected volume identities. Two contract-cycle attempts remain `INCONCLUSIVE` and eight remain `INVALID`, including the first control validation that exposed stale milestone vocabulary; the corrected control now passes. The exact candidate still requires commit, push, and credential-free public replay before U01 can close.

## 2026-08-11 - The release contract closes from public custody

Commit `f66409b3...`, tree `1aae10e6...`, was pushed and cloned with credential helpers and prompts disabled. The fresh public checkout restarted every U01 gate and reproduced the exact lock, plan, 141 tests, 53 mutations, one-schema/two-fixture offline meta-validation, milestone control, strict Git integrity, a zero-match four-pattern secret scan, missing approval at exit `12`, zero workflow execution, zero IncidentSeal containers and networks, and all three protected volume identities. The clone was clean and public `main` matched exactly; GitHub still had no release.

The first post-clone assertion guessed the wrong full expansion of the abbreviated commit and remains `INVALID`; the existing correct clone was verified in place. After verification, local command policy denied its cleanup wrapper before execution, so no bypass was attempted and that clean noncanonical clone remains in AppData temporary custody outside OneDrive, the canonical repository, and runtime. The first closure update then used `completed` where the current milestone vocabulary requires `complete`; the validator rejected it and the corrected control was rerun. All three wrappers remain `INVALID`. U01 now passes and U02 begins with the approved-workflow verifier; package building remains ineligible.

## 2026-08-11 - The approved-workflow runtime is narrowed before implementation

U02 contract lock `sha256:61ddf9be...` freezes a deliberately small v1 execution profile. Only Python and Node manifest steps are supported; host, PostgreSQL, Compose, and receipt runner values fail `INVALID` before Docker rather than becoming a general command platform. Source authority is the exact clean Git commit and SHA-256 of its null-delimited recursive tree listing. Only declared regular inputs are copied to an external read-only stage, persistent outputs are forbidden, and commands run as direct argv in exact locked non-root images with no network, secret, shell, host environment, broad mount, privilege, or socket.

The internal writer will retain canonical append-only events and content-addressed step evidence outside the repository and OneDrive, while the agent event path stays read-only. Resume is keyed to exact manifest and source identity; cancellation can stop only an exact labelled owned container. The contract passes 150 tests, all 47 mutations, its 14-file/14-binding lock, and one-schema/two-fixture full validation from six retained exact wheels without network. Approval stayed missing at exit `12`, no Docker or workflow ran, and nothing was built or published. One incorrect read-only Dockerfile path lookup remains `INVALID`. Public contract replay is next.

## 2026-08-11 - The workflow contract reproduces before implementation

Credential-free public commit `bcc8583e...`, tree `6d1cba99...`, repeated the exact lock, 150 tests, 47 mutations, and offline full-schema validation. Control, Git integrity, four zero-match secret patterns, clean worktree, remote equality, missing approval at exit `12`, and the exact Docker boundary passed. The aggregate post-replay integrity wrapper timed out without usable output and remains `INCONCLUSIVE`; each read-only component was rerun separately and passed. No workflow or product runtime started. The implementation stage may now begin, while operator approval remains deferred until a fixed real manifest is ready.

## 2026-08-11 - The first bounded verifier runs both exact language images

The local U02 candidate now implements the single-manifest `verify` path without adding approval authority or a general command platform. Approval is inspected before Git or Docker, source is bound to the exact clean origin, commit, and null-delimited tree digest, and committed declared blobs are copied into a new read-only stage. Python and Node commands run in the exact retained image IDs with inspected labels, numeric user, no network, read-only root, dropped capabilities, no-new-privileges, bounded processes, memory, and tmpfs. The repository, OneDrive, Docker socket, host environment, secrets, and persistent outputs remain outside runtime custody.

The distroless images contain immutable environment metadata and no shell or `env` utility. The implementation therefore uses a language-native no-shell bootstrap that clears the effective user-command environment to the four frozen names; Python supports the v1 script, `-m`, and `-c` profiles, while Node spawns with `shell:false`. A disposable Local AppData probe ran a dependency-ordered Python and Node workflow under temporary in-process test authority. Both steps passed, ten canonical events streamed at exit zero, a terminal receipt permitted the synthetic claim, every owned container was removed, and the production approval store was never written.

A second disposable probe then exercised nine state and recovery boundaries. Unexpected step exit stayed product `FAIL`; capture overflow stayed `INCONCLUSIVE`; changed authority became `stale`; an actual interrupt became `cancelled` with no verdict; an exact nonterminal key resumed; a process killed immediately after the durable `step.started` event left one container that was reobserved, safely removed, and replayed under the same run; terminal bytes remained unchanged; a different manifest digest started a different run; and an unowned lookalike was left untouched while the run became `INCONCLUSIVE`. Every probe-owned container was removed after evidence. Twenty focused tests and all 24 implementation mutations now pass. The real CLI remains `MISSING` / `INVALID` at exit 12; inherited-lock reconciliation, retained candidate receipts, a fixed operator approval, and public replay remain.

The full local reconciliation then passed all 160 repository tests, twenty current mutation programs, and 362 live file/digest bindings across 22 locks. The final two-runner and recovery probes repeated their PASS results, production approval remained `MISSING` / `INVALID` at exit `12`, zero IncidentSeal containers or networks remained, and all three protected volume identities matched their pre-attempt values. The candidate records separately retain two product failures—the overbroad Python effective environment and invalid-run-id event-stream exception—and five invalid evaluator or wrapper attempts, including an unquoted PowerShell revision expression during pre-commit integrity inspection. None is rewritten as a passing observation. Public implementation replay and a fixed operator-approved real manifest are still mandatory before packaging.

Exact public commit `6490c761...`, tree `95b77552...`, then reproduced the core verifier: 160 tests, all twenty mutation programs, 362 bindings, both pinned runners, ten PASS events, and all nine state/recovery cases. Production approval stayed missing, the four-pattern secret scan and strict Git check passed, the clone stayed clean and equal to public `main`, and Docker returned to the three exact protected identities. That public PASS is retained as `superseded` because it exposed the final missing presentation of authority: there was not yet a deterministic repository-owned way to project the exact clean commit and tree into the ignored real manifest that the operator would approve.

The additive candidate now supplies that narrow bridge. A clean-only materializer writes fixed `.incidentseal/workflow.json`, refuses silent replacement, retains changed prior bytes by digest, and never reads or writes approval. Its two-step manifest runs the static implementation validator in Python, then a separate Node program rehashes the 22-file lock and checks the authority, Docker, socket, secret, network, mount, hardening, evidence-state, active-unit, and pre-packaging gates. The first 29-case mutation run failed to reject removal of the clean-worktree branch and remains `INVALID`; the corrected validator rejects all 29. The current 163 tests, 23 focused tests, Node gate, dirty-worktree denial, and 365 lock bindings pass. Current public replay is next; operator approval remains deferred until the final exact public manifest exists.

Exact credential-free public commit `04fd0525...`, Git tree `6ea70c27...`, repeated the final implementation matrix. Its deterministic preview produced manifest digest `sha256:f8e074c5...` for null-delimited tree digest `sha256:67a2298b...`; the static and Node gates, 163 tests, all twenty mutation programs, both pinned runners, nine recovery/state cases, 365 bindings, missing authority, strict Git, four zero-match secret patterns, clean remote equality, and exact Docker custody all passed. The first clone wrapper used its not-yet-created destination as the tool working directory and remains `INVALID` without Git or runtime; the corrected clone holds the proof. The replay receipt now precedes the evidence-only commit, after which the final manifest can be materialized and presented for operator approval.

The evidence-only receipt reached public commit `ed70aede...`, and the deterministic manifest was materialized as digest `sha256:393f0d4f...` with approval still `MISSING`. The next live reconciliation correctly caught three stale current-state pointers before any human response: the status limitations still called the implementation local-only, the status next action still requested already-complete implementation work, and the control/milestone handoffs still pointed behind the public receipt. That unapproved handoff is retained as `superseded` with verification `INVALID`; it wrote no approval and ran no workflow. The smallest truth correction now precedes a new public commit and new exact manifest digest rather than asking the operator to approve a misleading state.

## 2026-08-11 - The real approved workflow passes without moving authority

The corrected truth commit reached exact public `main` at `3c47f16d...`, Git tree `6e8acdee...`, and null-delimited tree digest `sha256:d4c4d886...`. Deterministic rematerialization replaced the still-unapproved prior bytes under retained superseded custody and produced ignored manifest digest `sha256:969c0626...`. The operator approved those exact bytes through the TTY-only external approval path. Chat text was treated only as a signal to inspect authority; execution began only after `policy status` returned `MATCH`, `policy diff` returned no differences, and the external approval artifact independently rehashed to `sha256:56336362...`.

Real run `785847c3-059b-4361-aa5f-1b72b2d22e5d` completed through the stable checkout CLI. The Python implementation-lock step and independent Node release gate used their exact pinned image IDs, both exited zero, and both re-reported implementation lock `sha256:d58e5658...`. The external archive retained ten contiguous events with `MATCH` before each step, exact step record digests `sha256:71201f30...` and `sha256:0cd50681...`, and terminal receipt `sha256:75ff3e2a...` with lifecycle `completed`, verdict `PASS`, and reason `required-steps-passed`. Independent rehashing reproduced every content address and decoded both bounded outputs; stderr remained empty.

After the run, approval still matched, the tracked worktree was clean, no IncidentSeal container or network remained, and all three protected volume identities were unchanged. No package, image, tag, registry object, access setting, or release was created. The candidate records deliberately retain only hashes and calibrated claims; approval and run evidence remain external. Exact credential-free public replay of these repository-owned records is the final U02 closure gate before packaging starts.

The mandatory post-run repository suite then found a defect that pre-approval evaluation could not expose: three CLI tests asserted that the entire fixed production approval root did not exist. Legitimate approval therefore produced a real 160-pass/3-fail result even though the commands remained read-only. This is retained as `IS6-U02-IMPLEMENTATION-FAIL-003`, not collapsed into the earlier workflow PASS or rewritten as evaluator error.

The remediation does not add an authority override or test-only redirect. Each affected test snapshots every existing approval artifact by relative path, byte count, and SHA-256, executes the same fixture-specific read-only command, retains the expected fixture `MISSING` result, and requires the full production snapshot to remain exact. The focused 23 tests, complete 163 tests, 29 fail-closed mutations, static implementation validator, and independent Node gate now pass under candidate lock `sha256:6fefd277...`; no runtime started and the approved artifact remained unchanged. One first focused wrapper omitted `PYTHONPATH` and remains separately `INVALID`; the corrected locked environment passed. Because even a test-only source correction changes the exact lock, source commit, tree, and manifest, the retained real PASS cannot authorize the new candidate. Public replay, exact rematerialization, matching operator approval, and another real workflow are required before U02 closure.

Credential-free public commit `3d10dcd...`, tree `b599fbf6...`, then reproduced the remediation with the legitimate approval store still present: all 163 tests, 23 focused tests, 29 mutations, the static Python validator, independent Node gate, and controlled milestone passed under lock `sha256:6fefd277...`. Strict Git integrity, a four-pattern zero-hit secret scan, clean remote equality, exact rehashes of the retained approval, event archive, both step records, and terminal receipt, zero containers or networks, and the three exact protected volume identities passed. The check-only materializer projected manifest digest `sha256:36adcb6c...` without writing or approving it. One unsupported `--preview` wrapper remains public `INVALID`; corrected `--check` passed. The receipt itself must now reach public custody before the final manifest is rematerialized.
