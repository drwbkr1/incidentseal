...
Progress: `IS6-U01` passes from exact public custody. The U02 runtime-free execution contract passes from exact public commit `bcc8583e...`, tree `6d1cba99...`; its unchanged machine contract is re-bound under lock `sha256:172cb06a...`. Current implementation lock `sha256:d58e5658...` covers the gated `verify` command, copy-only staging, pinned Python and Node containers, external event custody, receipts, recovery, deterministic ignored real-manifest materialization, and an independent Node release gate. Exact credential-free public commit `04fd0525...`, tree `6ea70c27...`, reproduced all 163 tests, twenty mutation programs including all 29 implementation mutations, 365 bindings, deterministic manifest preview, the Node gate, both real runners, nine recovery/state scenarios, missing approval, Git/secret integrity, and exact Docker custody. The prior `6490c761...` public PASS remains superseded. Two product failures, seven invalid attempts across local/public evaluation, and one superseded PASS remain separately retained. The final fixed operator-approved manifest and real workflow replay remain before U02 can close. No package, image, tag, or release has been built or published.
