...
Exact credential-free public commit `2e22804aef49071ef42611357eaa4a7f4e0f384e`, tree `73c2480aa4e700b506ef5b00202e9ce6ee2fc702`, reproduced the lock, projection, corpus, all 37 mutations, all 111 tests, and both schemas/four fixtures from six exact wheels without network. Strict Git integrity and four high-confidence secret patterns passed with zero matches. Approval invocation `9cb57896...` remained `MISSING` / `INVALID` at exit `12`; the clone stayed clean; no workflow ran; no server, browser, container, network, or disposable volume appeared; and all three protected volume identities remained exact. One public assertion-wrapper attempt remains `INVALID` after looking in the wrong envelope object; the corrected read-only assertion passed. `EXIT-DASHBOARD-CONTRACT` now passes and `IS5-U02` is eligible.

## IS5-U02 local read-only dashboard implementation candidate

Implementation lock `sha256:53e89d0bd182da7f15d899b75f338318b9ff10b01689ffe033e198021f6f6447` binds a separate Windows/POSIX launcher, dependency-free server, local CSS and JavaScript, static validator, real-surface harness, tests, and 29 implementation mutations. It also hash-binds the unchanged verification CLI and the public-reproduced dashboard contract. The HTTP surface exposes only `/`, two local assets, `/api/snapshot`, and `/healthz` over IPv4 `127.0.0.1` with an exact active-port Host header.

The exact Windows launcher served 25 bounded real requests on port `54007`: five GETs, five HEADs, seven denied write/control methods, four denied Host attacks, and four denied query/traversal targets. Every response carried eight fixed defensive headers. Response bytes matched the locked HTML, CSS, JavaScript, canonical snapshot, and health digests. The process made zero external requests and repository writes, then closed its socket and left zero dashboard processes. Nine evaluator-only scenario views are distinct, text-labelled, and claim-calibrated; they cannot be selected through HTTP or launcher input.
...
## IS5-U04 local repeated-evaluation candidate

Evaluation lock `sha256:c75fe0f9ddca14326ddfe0ad233d73853c671b99bf2a5cfa4872c569c4bbcdd2` freezes a no-argument, dependency-free 27-trial plan. Each trial strictly reloads and validates all seven checkpoint records, generates one fixed semantic HTML document, starts a new IPv4-loopback server, verifies all five read-only response bodies and security headers, and closes the server before the next state. The full saved dataset passes 27/27 case semantics, 135/135 requests, 189/189 source observations, nine stable HTML identities, and three crash-to-recovery transitions without claim promotion. Exactly three success trials permit claims; all 24 non-success trials withhold them, yielding zero false PASS or release claims.

The local candidate reports observations, not portable budgets: projection latency ranged from 4.362600 to 6.540900 ms with a 5.174400 ms median; deterministic server-side HTML generation ranged from 0.038100 to 0.053100 ms with a 0.040700 ms median; peak process memory ranged from 28,745,728 to 29,470,720 bytes; and five-route response totals ranged from 30,462 to 30,552 bytes. Browser paint is explicitly outside this measurement and remains covered by U03.

All 133 tests and 30 result mutations pass, as do the 37 contract, 29 implementation, and 28 browser mutations. Approval invocation `5b5f95cf...` stayed `MISSING` / `INVALID` at exit `12`; no workflow ran; no Docker surface was accessed by the evaluator; every server closed; zero IncidentSeal containers or networks exist; and the three protected volume identities remain exact. Five invalid wrappers and one pre-lock passing calibration retained as `superseded` remain distinct. This is a local candidate only: exact public replay remains before `EXIT-SCENARIO-CORPUS` or `EXIT-EVALUATION-REPORT` can pass.
